...
import random
from pymetheus.buffer import SumTree
...
class TestSumTree(unittest.TestCase):
    ...
    def _validate_structure(self):
        ''' Validates the sum-tree property of the buffer. '''
        for i in range((len(self.buffer)-1)//2):
            left, right = 2*i+1, 2*i+2
            if self.buffer._tree[i] != (self.buffer._tree[left] +
                                        self.buffer._tree[right]):
                return False, (
    ...
    def test_init(self):
        ...
        self.assertEqual(len(self.buffer._tree), 2*self.capacity-1)
    ...
    def test_clear(self):
        ...
        self.assertTrue(
            all(torch.equal(self.buffer.proto_experience[key],
                            self.proto_experience[key])
                for key in self.proto_experience.keys()))
        self.assertEqual(self.buffer._tree, [0.0] * (2*self.capacity-1))
        self.assertEqual(self.buffer._size, 0)
        self.assertEqual(self.buffer.total, 0.0)
        self.assertEqual(self.buffer._next, 0)
    ...
    def test_sample(self):
        for priority in [random.randint(1, 100) for _ in
                         range(self.capacity-len(self.buffer))]:
            experience = self._make_item(priority, len(self.buffer))
            self.buffer.add(experience)
        n = 100
        k = 10
        e_dist = [
            self.buffer._tree[i+self.buffer.capacity-1] / self.buffer.total
