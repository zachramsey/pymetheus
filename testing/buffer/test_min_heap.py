...
import random
from pymetheus.buffer import MinHeap
...
class TestMinHeap(unittest.TestCase):
    ...
    def _validate_structure(self):
        ''' Validates the min-heap property of the buffer. '''
    ...
    def test_init(self):
        ...
        self.assertEqual(len(self.buffer._heap), len(self.init_priorities[1:]))
    ...
    def test_clear(self):
        ...
        self.assertTrue(
            all(torch.equal(self.buffer.proto_experience[key],
                            self.proto_experience[key])
                for key in self.proto_experience.keys()))
        self.assertEqual(len(self.buffer._heap), 0)
